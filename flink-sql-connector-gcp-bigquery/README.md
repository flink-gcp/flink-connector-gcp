...
the `shade-flink` execution inherited from `flink-connector-parent` — and, more directly, the
sibling `flink-sql-connector-gcp-pubsub` in this repository, which established the shading and
licensing decisions this module reuses. Those are **design references only**; the shade
configuration here was written for this project.
