...
/** Request and client lifecycle recorder shared by read-modify-write API tests. */
final class FakeReadModifyWriteClients implements SingleRowClientFactory, SingleRowClient {
    private static final long serialVersionUID = 1L;
    final List<ReadModifyWriteRow> sent = new ArrayList<>();
    final List<SettableApiFuture<Row>> answers = new ArrayList<>();
    ...
    public ApiFuture<Row> readModifyWriteRow(ReadModifyWriteRow request) {
        sent.add(request);
        SettableApiFuture<Row> answer = SettableApiFuture.create();
        answers.add(answer);
        return answer;
    }
}
