...
/**
 * Mints the {@link RowKeySampler} one enumerator plans against.
 *
 * <p>Serializable because the source carries it there; the sampler it mints is not, which is the
 * whole reason for the indirection. The JobManager holds one source object for a job's whole life,
 * so a sampler on the source configuration would be shared by every enumerator a coordinator reset
 ...
 * the sampler it answers with builds one on first use, so a restore whose checkpoint already
 * records a plan mints a sampler and opens nothing.
 */
@Internal
public interface RowKeySamplerFactory extends Serializable {
     ...
     *
     * @return the sampler, owned by the caller, which closes it exactly once
     * @throws IOException if the sampler cannot be created
     */
    RowKeySampler create() throws IOException;
}
