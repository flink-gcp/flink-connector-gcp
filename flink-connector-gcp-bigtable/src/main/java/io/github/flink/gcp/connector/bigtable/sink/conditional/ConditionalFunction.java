...
@Internal
final class ConditionalFunction<T>
        extends BigtableRequestFunction<T, Boolean, Tuple2<T, ConditionalResult>> {
    private static final long serialVersionUID = 1L;
    private final ConditionalConfig<T> config;

    ConditionalFunction(ConditionalConfig<T> config) {
        super(
                config.appProfileId,
    ...
    ConditionalFunction(ConditionalConfig<T> config, SingleRowClientFactory factory) {
    ...
    @Override
    protected RowRequest<Boolean> request(T input) throws Exception {
        ConditionalRequest request = config.serializer.serialize(input, null);
        return request == null
                ? null
                : new ConditionalRowRequest(request, config.emptyBranchPolicy);
    }

    @Override
    protected Tuple2<T, ConditionalResult> result(
            T input, Boolean answer, TableDestination destination, RowRequest<Boolean> request) {
        ConditionalRowRequest conditional = (ConditionalRowRequest) request;
        return Tuple2.of(
                input,
                new ConditionalResult(
                        ...
                        request.rowKey(),
                        answer,
                        conditional.selectedBranchHasMutations(answer)));
    }
}
