...
/**
 * Bounded read-modify-write requests emitting the original input and its successful response.
 * Delivery is at-least-once; a replay can apply an operation again. No async retry entry points or
 * failure handlers are provided. Requires single-cluster routing and single-row transactions.
 *
 * @param <T> the input type
 */
@PublicEvolving
public final class BigtableReadModifyWriteAsync<T> {
    private final ReadModifyWriteConfig<T> config;

    BigtableReadModifyWriteAsync(ReadModifyWriteConfig<T> config) {
        this.config = config;
    }
     ...
     * @return the builder
     */
    public static <T> BigtableReadModifyWriteAsyncBuilder<T> builder() {
        return new BigtableReadModifyWriteAsyncBuilder<>();
    }
     ...
     */
    public SingleOutputStreamOperator<Tuple2<T, ReadModifyWriteResult>> orderedWait(
     ...
     * @return input/response pairs; serializer skips emit nothing
     */
    public SingleOutputStreamOperator<Tuple2<T, ReadModifyWriteResult>> unorderedWait(
            DataStream<T> input, Duration timeout) {
    ...
    ReadModifyWriteFunction<T> function() {
        return new ReadModifyWriteFunction<>(config);
    }
    ...
    private TupleTypeInfo<Tuple2<T, ReadModifyWriteResult>> resultType(DataStream<T> input) {
        return new TupleTypeInfo<>(
                input.getType(), TypeInformation.of(ReadModifyWriteResult.class));
    }
}
