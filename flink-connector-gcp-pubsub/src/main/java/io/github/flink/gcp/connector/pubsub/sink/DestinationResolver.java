...
/**
 * Resolves the destination topic for each record, enabling one sink instance to publish to many
 * topics (dynamic destinations).
 *
 * <p>The resolver is invoked once per record on the hot write path. Implementations must be
 * deterministic and cheap; when destinations repeat, return cached {@link TopicDestination}
