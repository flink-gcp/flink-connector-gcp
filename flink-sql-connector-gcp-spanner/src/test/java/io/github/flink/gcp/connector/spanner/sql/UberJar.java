...
final class UberJar {
    ...
    static final String FACTORY_CLASS =
            "io.github.flink.gcp.connector.spanner.table.SpannerDynamicTableFactory";

    /**
     * A floor below which a parsed artifact list is assumed to be a parsing failure rather than a
     * real bundle; the bundle holds 73 artifacts today. Set a little below that: it exists to catch
     * "the regex matched nothing", not to pin the count, which the NOTICE comparison itself already
     * does exactly.
     */
    static final int MINIMUM_BUNDLED_ARTIFACTS = 60;
