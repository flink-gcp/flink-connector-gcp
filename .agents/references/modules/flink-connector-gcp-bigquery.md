...
- The table source pushes down top-level projection through `selectedFields(...)`, including the
  ...
  Keep both Boolean selection and compound-fragment rendering iterative; bounding generated bytes
  does not bound recursive stack consumption. Preserve original grouping and visit order rather
  than flattening or rebalancing expressions. Flink has separate measured failures on deep inputs
  (ADR-0100).
