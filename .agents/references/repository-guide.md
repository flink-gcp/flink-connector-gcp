...
- **SQL guidance in examples, quickstarts, and Table connector references renders one tested source
  region** (#1097, #1126; ADR-0144).
  ...
  carries no second copy. The 32 Table connector reference regions comprise 27 complete positive
  examples, four partial fragments, and one intentionally invalid example. Complete examples reach
  their documented command, catalog or planner boundary; a parser-only session command also asserts
  the exact command-specific value it publishes. Minimal test-only enclosures validate fragments
  without rendering invented context, and the negative example must fail with its documented
  connector diagnostic after earlier schema checks pass. Use
...
- **Flink 1.20 (1.x LTS) is supported from this same source, at source level** (ADR-0054): the
  ...
  ADR-0160 also places the test-utils lineage listener in `src/main/java-flink2` and its
  consumer graph/planner tests in `src/test/java-flink2`; neither compiles for 1.20
...
- **The test frameworks follow Flink across a major.** The root POM imports `junit-bom` and
  ...
  **Flink's own test utilities declare** — `flink-test-utils-junit` 2.2.1 declares
  ...
  not on Central
- **The Jackson set is managed by the root POM's `jackson-bom` import, first among the imports**
  so it wins over any BOM that later starts managing Jackson; a module that needs a Jackson
  artifact names it without a version. The version pairs with Avro's pin (next bullet); ADR-0150
  has the history
- **A version pinned to match another project's pin carries an `ignore` rule for every update type**,
  ...
  `jackson-bom` follow what `avro-parent ${avro.version}` pins (reason on each property in the
  root POM), and
...
- `sink.<writepath>` is one subpackage per write-path **family** (internal stages as `.writer`,
  ...
  rule is a test, not a count. When a second family arrives, both runtimes take a family layer;
  the existing runtime's move may land separately to avoid racing functional changes.
  Bigtable's writer stages now live in `sink.mutaterows.writer` and `sink.singlerow.writer`;
  that move preserves the existing entry-point layout (ADR-0148)
