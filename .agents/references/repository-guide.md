...
- **`pinact.yaml` is deliberately not enrolled in that gate** (ADR-0121): it needs the App
  credentials, which fork pull requests do not get, and a required check that cannot pass for a
  fork blocks every outside contribution. It reports as its own non-required check
