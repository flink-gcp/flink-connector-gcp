...
`TIME` is capped at millisecond precision because `RowData` stores it as an integer of
milliseconds. Temporal values are truncated to the precision declared by the DDL;
`TIMESTAMP(6)` and `TIMESTAMP_LTZ(6)` preserve BigQuery's microseconds.
