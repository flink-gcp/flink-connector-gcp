...
The destination fields assemble `DatabaseDestination`, the physical DDL supplies the serializer, and the nine `sink.*` options map one-for-one onto `SpannerWriterOptions`.
