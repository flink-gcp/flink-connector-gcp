...
- The two bullets that name quantities do not name all of them. Checkpoint duration and size, live TaskManager heap, serialization and restore copies, pending checkpoints, backlog and the remaining replay budget a steady-state cell reports — which is not the budget a recovery consumes, deferred above — are to be measured on this rig by the shape that produces them, and the revision above is where each is assigned; recovery time and the budget a recovery consumes wait on the revision named above, which is where the failure they are measured under is fixed.
...
- The assessment keeps its own resource and cost approval, and no ceiling for this rig exists yet. Neither the move nor this refinement authorizes a session.
