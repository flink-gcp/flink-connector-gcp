...
Enrolment is opt-in under [ADR-0059](0059-ci-yaml-orchestrates-pull-request-ci-behind-one-required-check.md), and a required check that depends on a secret forks cannot have would block every outside contribution.
...
Two files in the checkout are trusted input for the same reason: `.pinact.yaml` decides what pinact scans, and an `aqua.yaml` at the repository root precedes the action's own checksum-pinned configuration.
Both are in the trigger's paths so a change to either at least runs the check, and both are review surface, not a control.
...
- **A personal access token**: it binds automation to one person's account and leaves with them, and its scope is the account's repositories rather than one installation. Not for commit signing — pinact-action commits through the Git Data API without a signature either way, so neither produces verified commits.
