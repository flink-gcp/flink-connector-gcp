...
- HTTP targets only (App Engine targets are region-locked, invert 429/503 overload behaviour and
  are "less common"); targets need an external IP; OIDC vs OAuth is chosen by the target, not by
  preference, so the builder rejects setting both. Fixed **and** per-record queue destinations
  from v1 — unlike Pub/Sub topics and BigQuery tables this costs nothing, since one
  `CloudTasksClient` serves every queue with no per-destination stream.
...
- `BatchCreateTasks` and `BufferTask` are **both REST-only and absent from the Java client**,
  ...
  **cannot be detected through the v2 client at all** (the field does not exist in the v2
  proto).
- The serialization schema is a **two-stage builder**: `httpTarget(url)` returns a non-generic
