...
The supervisor Job tolerates two Pod replacements, because a preemption in the Pod's first seconds is the infrastructure reclaiming a full node rather than a verdict on the run; the replacement takes the run over only while no cell has been claimed, and the control record names the Pod that holds it, which is also what revokes the replaced Pod: a supervisor that no longer holds the run stops without cleaning up.
