...
- **A stop-with-savepoint that does not reach FINISHED.** Flink notifies a synchronous savepoint complete, so the committer creates its tasks, but does not add the savepoint to the completed-checkpoint store; if any task then fails to finish, whether the commit itself or an unrelated operator, Flink fails the job without automatic recovery. An external restart from the older checkpoint re-stages the savepoint's records under new random names (§ Terminal behavior and runbook).
...
If the job then does not reach FINISHED, because the commit threw or because any other task failed after the savepoint completed, Flink's stop-with-savepoint handler calls global failure handling with a `StopWithSavepointStoppingException`.
On both supported lines this exception carries `@ThrowableAnnotation(ThrowableType.NonRecoverableError)`, so `ExecutionFailureHandler` fails the job instead of restarting it, even with a configured restart strategy.
If an operator or deployment automation instead restarts from the last completed checkpoint N, the source replays every record between N and the savepoint, the writer stages them again, and records without a stable key receive new random names, so the tasks the savepoint's commit had created, some or all of them, exist twice.
...
A stop-with-savepoint that does not reach FINISHED is therefore outside the guarantee; the runbook is to preserve the savepoint path in the exception and resume from that savepoint, preventing external automation from choosing the older checkpoint. If an external restart has already happened, cancel it before its next checkpoint completes and resume from the savepoint, whose committer state re-commits the same names, and the documentation says that a stable key removes the hazard.
...
Both new types are `@Experimental`, as signature-closure stops on the `@Public` builder recorded in ADR-0124, under [ADR-0141](0141-a-surfaces-stability-tier-is-set-by-what-can-reshape-its-inputs-and-outputs.md)'s youth clause and signature-closure rule: the mode has not survived a published release, the deferred committer-side failure handler and dynamic destinations (§ Alternatives declined) would reshape the options' validation, and the label is conditional on the gates below.
The builder method and the planned Table key are the entry surface and stay on the `@Public` builder and factory.
The implementation corrects this ADR's earlier `@PublicEvolving` choice: ADR-0141 allows an evolving group only without outside frozen referrers, and explicitly rejects mixing an evolving parameter into a frozen signature.
The new options builder and expiry-policy enum share the experimental tier; renaming the top-level types still affects the frozen builder signatures and requires the ordinary compatibility decision.
...
The committer registers `tasksDeduplicated`, error-class and optional destination counters, and counters for each expiry override.
The override counters distinguish assumed completion and dropping from actual collision responses; `CREATE_ANYWAY` counts authorized attempts, including retries, rather than successful physical creations.
Flink additionally supplies standard committer metrics: the pending-committables gauge that its collector registers per committer subtask and the already-committed counter that `signalAlreadyCommitted()` feeds.
...
| A synchronous savepoint is notified complete but is not added to the completed-checkpoint store; a stop-with-savepoint whose tasks do not finish fails the job non-recoverably | `CheckpointCoordinator.completePendingCheckpoint` (`isSavepoint` excludes the store, `isSynchronous` sends the notification), `StopWithSavepointTerminationHandlerImpl`, `StopWithSavepointStoppingException`, `ExecutionFailureHandler` | Both lines mark the stop failure non-recoverable. An external restart from the older checkpoint replays the savepoint's records (§ Terminal behavior and runbook). |
...
### Production committer implementation (2026-09-07)

Issue #1243 adds the DataStream mode, immutable public staging options, bounded committer loop, graph checks and v2beta3 retention readback.
The existing version-1 envelope remains unchanged.
Production GAX calls carry the same absolute gRPC deadline created before the wall-clock authorization check; callbacks can only enqueue their own attempt's result.
The committer owns both retry budgets, excludes late callbacks, interrupts and cancels outstanding work on close, and signals only collision success or the explicit expired-envelope override.
Standard Flink committer counters are observed through the production operator harness, while custom collision, error-class and optional destination counters are updated on the committing thread.
The public DataStream guide includes configuration, checkpoint/heap sizing, state-loss hazards and an expiry/savepoint runbook with a compiled configuration example.
The v2beta3 retention client and settings are class-level `@BetaApi` in the pinned 2.96.0 SDK.
...
| End to end, every record reaches the handler, including the final bounded batch | The existing emulator job case under `EXACTLY_ONCE` | The distinct dispatched bodies differ from the input records. Dispatch can repeat independently of creation; accepted-name oracles in the fault tests hold unique creation. |
