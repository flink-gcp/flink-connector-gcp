...
| `serializer` | **required** | Converts each record into a `PubsubMessage`, or into `null` to skip it. `dataOnly(...)` wraps a payload-only schema; `withAttributes(...)` and `withOrderingKey(...)` layer onto any of them |
