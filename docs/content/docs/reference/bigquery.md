...
| `maxConcurrentCheckpointFinalizations` | 1 | Maximum open staging files each writer finalizes concurrently at a checkpoint or end of input. Accepted values are 1 through 8. It helps only when one writer holds multiple destination files; size rolls, capacity evictions, and idle closes remain serial |
| `maxConcurrentDestinations` | 8 | Maximum destination actions executed concurrently by the single committer. Concurrently dispatched actions use worker threads and BigQuery REST clients. A singleton action uses the committer thread until the worker pool has been initialized; later singleton phases reuse that pool. Accepted values are 1 through 64 |
...
| `selectedFields` | every column | The columns to read. Applied by BigQuery when the session is created, so the rest are neither transferred nor included in the metered bytes-read usage |
| `rowRestriction` | no filter | A BigQuery filter expression — a `WHERE` clause without the keyword — applied before any row is sent. The Table API combines this with any generated SQL prefilter using parenthesized `AND` |
