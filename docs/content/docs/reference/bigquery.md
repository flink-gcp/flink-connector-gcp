...
| `failedRowHandler` | `FailedRowHandler.failJob()` | What happens to a row that terminally fails — fail, drop, or dead-letter |
...
| Option | Default | What it does |
|---|---|---|
| `stagingPath` | **required** | The `gs://` prefix staged Avro files are written to |
