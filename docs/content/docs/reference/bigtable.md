...
| `deliveryGuarantee` | `AT_LEAST_ONCE` | Selects eager writes or experimental checkpoint-owned `EXACTLY_ONCE`; the latter makes a row readable one checkpoint interval later at best plus the commit drain, so the Stage 2 assessment declined the performance gate on 2026-09-21 under [#1327]({{< param BookRepo >}}/issues/1327) and claims no supported workload; it ran the committer below its default concurrency, which [#1464]({{< param BookRepo >}}/issues/1464) measures |
