...
| `just check-readme-examples` / `just check-doc-snippets` | Source-backed Java examples stay synchronized with their tagged source regions and compile against the working tree; the first covers the module READMEs, while the second also builds the production Hugo site, adds the docs-page and Javadoc halves, and plans every source-backed Flink SQL region. A deliberately abbreviated Java example carries its classification marker and says so in prose |
| `just check-doc-sql-snippets` | Every code block labeled `sql` in examples and quickstarts is source-backed; Flink SQL is parsed and planned against the current connector factories, while GoogleSQL DDL and seed statements execute against the Spanner emulator |
