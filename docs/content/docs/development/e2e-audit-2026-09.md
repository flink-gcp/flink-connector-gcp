...
BigQuery table cleanup attempts every named table, and a failed table cleanup does not skip a separate staging-prefix cleanup.
...
No retries, skips, weaker assertions or production connector APIs are introduced.
...
| `BigQueryMultiStreamRealGcpITCase` | Weekly | Partition-distribution, count and restart/savepoint probes; preserves timestamp and bounded-checkpoint repairs. |
