...
Every option is listed on the [Spanner options]({{< relref "docs/reference/spanner" >}}) page.
...
## Batch limits and metrics

This section covers write-batch correctness bounds first, then the metrics that show which limit
binds and how the source and sink behave.
...
| `FLOAT64` / PostgreSQL `FLOAT8` keys accept `NaN` in the emulator | The real service rejects these values in primary and secondary key columns. The float-key emulator test records acceptance in both dialects; the gated float-key read suite checks rejection separately from its numeric-range matrix. PostgreSQL's [type contract](https://cloud.google.com/spanner/docs/reference/postgresql/data-types) states the restriction explicitly |
...
The [module README]({{< param BookRepo >}}/blob/main/flink-connector-gcp-spanner/README.md) records implementation status and provenance.
The connector-specific design decisions on this page link to their durable ADRs.

### Not here yet

- `MUTABLE_KEY_RANGE` partition-event records; the Change Streams source supports
  `IMMUTABLE_KEY_RANGE` and rejects another mode at startup.
