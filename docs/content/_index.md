...
|---|---|
| BigQuery | [Source and sink]({{< relref "docs/connectors/datastream/bigquery" >}}) — bounded table and query reads over the Storage Read API; writes through the Storage Write API (at-least-once and exactly-once) and GCS-staged load jobs, with dynamic per-record table destinations. Experimental [CDC sink]({{< relref "docs/connectors/datastream/bigquery" >}}#change-data-capture) supports upserts and deletes on the at-least-once default stream. Source, sink, and CDC writes are also available [from SQL]({{< relref "docs/connectors/table/bigquery" >}}) |
