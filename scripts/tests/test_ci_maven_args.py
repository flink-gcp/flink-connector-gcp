...
@pytest.mark.parametrize(
    ...
    [
        ...
        "tools/tier3/pyproject.toml",
        "tools/tier3/src/flink_tier3/cli.py",
        "tools/tier3/tests/test_package.py",
        "pyproject.toml",
        "uv.lock",
...
def test_the_workflow_reads_exactly_the_keys_the_lanes_carry(ci_maven_args):
    """The seam between this script and verify.yaml, which nothing else holds.

    GitHub resolves an unknown matrix property to the empty string rather than failing,
    so a rename on either side is silent *and* green in both directions: an empty
    `matrix.lane.args` makes `just verify` build the whole reactor on every lane, and an
    empty `matrix.lane.notice` turns the NOTICE check off for every shaded module
    indefinitely. Held here for the same reason test_ci_gate.py parses the real workflow
    files rather than a copy of them.
    """
    workflow = (SCRIPTS.parent / ".github" / "workflows" / "verify.yaml").read_text()
    referenced = set(re.findall(r"matrix\.lane\.([a-z_]+)", workflow))
    emitted = {
        key
        for lane in ci_maven_args.split_into_lanes(ci_maven_args.pom_modules())
        for key in lane
    }
    assert referenced <= emitted, (
        f"verify.yaml reads {referenced - emitted}, never emitted"
    )
    assert "args" in referenced, "the workflow no longer passes the lane's -pl list"
