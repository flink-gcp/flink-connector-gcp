...
public final class BigQueryConnectorOptions {
    ...
    /**
     * The Google Cloud project owning the destination table. Given as a bare project id, not a
     * resource path.
     */
     ...
     * Not required for a source query.
     */
    ...
    /**
     * The direct source or destination table, as a bare table id. Not required for a source query.
     * One destination per SQL table: per-record routing has no SQL surface and stays on the
     * DataStream API.
     */
     ...
     * Connects over plaintext without credentials, so it is for testing only.
     */
    ...
    /**
     * A BigQuery emulator's REST endpoint as 'host:port', for source query or view materialization
     * and sink table metadata. Separate from 'emulator-endpoint' because BigQuery serves the two
    ...
    /**
     * A path at which the same service-account JSON key is available to every Job Manager and Task
     * Manager that opens a BigQuery or Cloud Storage client.
     */
    ...
    /**
     * The project that owns and is billed for the Storage Read session; set it when reading a table
    ...
    /**
     * A GoogleSQL query whose result is read instead of the configured table. Dataset and table are
     * not required.
     */
    ...
    /**
     * Whether a configured table that is a logical or materialized view is materialized through a
     * query before it is read.
     */
    ...
    /**
     * How long a re-planned source may reattach to the same query job. Requires
     * scan.query-location.
     */
    ...
    /**
     * Which write path the sink uses. Each carries its own tuning family — 'sink.default-stream.*',
     * 'sink.buffered-stream.*' and 'sink.file-loads.*' — and a key of a family this option does not
     * select is rejected rather than ignored.
     */
     ...
     * Setting it avoids a metadata lookup when a write connection is opened and locates CDC
     * maximum-staleness jobs; under FILE_LOADS it is the location load jobs run in.
     */
    ...
    /**
     * Whether the default-stream sink writes the table changelog as BigQuery CDC UPSERT and DELETE
     * mutations. Requires a declared primary key and storage-api-at-least-once. With
     * create-if-needed, the sink creates and verifies the CDC table before writing.
     */
    ...
    /**
     * The Debezium MySQL source UUIDs in causal order. The first UUID has epoch one; append a new
     * UUID after a non-interleaved failover, and never edit or reorder existing entries.
     */
    ...
    /**
     * The TiCDC cluster ID whose commit timestamp oracle values order this table, as the
     * changefeed's server reports it in 'cluster_id'. A row change reporting another cluster is
     * rejected rather than ordered against an unrelated oracle. TiCDC defaults this identifier to
     * 'default', so give each TiCDC cluster its own before routing more than one into one table.
     */
    ...
    /**
     * Whether the desired CDC table state has maximum staleness disabled. Mutually exclusive with
    ...
    /**
     * Whether the sink may add columns to the destination table when the DDL schema carries a
    ...
    /**
     * Whether the sink may relax a REQUIRED column of the destination table to NULLABLE. BigQuery
     * cannot walk the reverse change back.
     */
    ...
    /**
     * The TIMESTAMP, DATE or DATETIME column a created table is partitioned on. Requires
    ...
    /**
     * The columns a created table is clustered on, in precedence order. BigQuery takes at most
     * four, and they must be top-level columns of the table.
     */
    ...
    /**
     * Whether a column declared NOT NULL derives a REQUIRED BigQuery column. Off, every derived
     * column is NULLABLE — REQUIRED is the mode BigQuery cannot walk back, so it is opted into
    ...
    /**
     * Dotted paths of STRING or ROW columns to derive as BigQuery JSON columns, for example
     * 'payload' or 'event.body'. A map value's path segment is 'value'.
     */
    ...
    /**
     * Dotted paths of STRING columns to derive as BigQuery GEOGRAPHY columns. The values are passed
     * through unvalidated, as on every other write path.
     */
    ...
    /**
     * The size at which the sink splits a batch into several append requests, below the Storage
     * Write API's own request limit.
     */
    ...
    /**
     * The first backoff of the connector's own repair loop, which reopens a write connection after
     * a transient failure.
     */
    ...
    /**
     * The first delay of the client library's own in-stream append retries, a layer below the
     * repair loop above.
     */
    ...
    /**
     * The wall-clock budget the client library spends retrying one append, whichever attempt it is
     * on.
     */
     ...
     * JVM-global in effect: the first writer's value wins for the whole connection pool.
     */
    ...
    /**
     * The connection pool's floor per region. Applied once per JVM, by whichever sink opens a
     * connection first.
     */
    ...
    /**
     * How long a destination may go unwritten before its write connection is closed. Set a large
     * duration to disable the sweep.
     */
    ...
    /**
     * A periodic flush, for jobs whose checkpoint interval is long or absent. A mitigation only:
     * the delivery guarantee still rests on checkpointing.
     */
    ...
    /**
     * Whether the sink reports its write metrics per destination table as well as in total. One SQL
     * table writes to one destination, so this mainly adds the table name to the metric group.
     */
    ...
    /**
     * How long a checkpoint-clean destination may go unwritten before its local appender and writer
     * state are dropped. A later row creates a new buffered stream.
     */
    ...
    /**
     * The first backoff of the connector's own recovery schedule, which creates the stream after a
     * table auto-creation, reopens it after a transient failure and drives the restore probe.
     */
    ...
    /**
     * The Cloud Storage path staged files are written under, of the form gs://bucket or
     * gs://bucket/prefix. Required under the 'file-loads' write method and rejected under the
     * others.
     */
     ...
     * 'write-append' only, since every checkpoint commits its own staged files. In batch,
     * 'write-truncate-data' replaces rows while preserving the destination schema and constraints.
     */
    ...
    /**
     * The smallest checkpoint interval streaming execution accepts, checked when the job graph is
     * built. BigQuery allows 1,500 modifications per standard destination table per day, and each
     * checkpoint issues a direct load or an overflow copy, so lowering this is an explicit opt-in
     * for a job whose daily count stays safe.
     */
    ...
    /**
     * The size at which an open staging file is finished and the next one opened. Raise it for a
     * very large volume to one destination, which the 10,000-URI per-load-job cap would otherwise
     * push onto the temporary-table plus copy path; lowering it buys little, because measured load
     * time climbs steeply below about 8 MiB per file.
     */
    ...
    /**
     * How Parquet staging files are compressed. Rejected when the staging format is Avro, whose
     * codec is not configurable. 'none' is the only value that needs no Hadoop runtime, and it
     * stages substantially more bytes than Avro does.
     */
    ...
    /**
     * The first backoff between polls of a submitted load, copy, or terminal query job. Lowering it
     * notices a finished load sooner, at the cost of more jobs.get calls.
     */
    ...
    /**
     * How many attempts a schema reconciliation makes. Only a lost race consumes one, so raise it
     * when something outside this job updates the same table concurrently.
     */
