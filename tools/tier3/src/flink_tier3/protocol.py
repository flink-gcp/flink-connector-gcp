...
# The highest offered rate a cell may declare, and the ceiling of the search.
MAX_RATE = 10000
...
def session_cell(cell, offered_rate, suffix="", control_delay_millis=0, emit=True):
    ...
    if not isinstance(offered_rate, int) or not 1 <= offered_rate <= MAX_RATE:
        raise Failure(f"Offered rate must be an integer from 1 through {MAX_RATE}")
    result = {
...
def next_probe(state):
    ...
    Returns ``(status, rate)`` with status ``searching``, ``bracketed``,
    ``frozen`` or ``ceiling``; ``rate`` is the next offered rate, or the
    frozen rate. ``ceiling`` means the highest rate the protocol admits was
    itself accepted, so the search has no further probe to ask for and the
    capacity is reported as at least that rate.
    """
    ...
    if rejected is None:
        if accepted >= MAX_RATE:
            return "ceiling", MAX_RATE
        return "searching", min(MAX_RATE, accepted * 2)
    if accepted is None:
        if rejected <= 1:
            return "frozen", 0
