...
def observation_window(placed, completed_ids, start_ms, warmup_seconds, last_ms):
    ...
    first = next((c for c in ordered if c["trigger"] >= warm_end), None)
    ...
    if len(inside) < MIN_CHECKPOINTS or last["ack"] <= first["trigger"]:
        return None, "window-too-short"
    return {
        "clock": "supervisor",
        "start": first["trigger"],
        "end": last["ack"],
        "seconds": last["ack"] - first["trigger"],
...
def analyze_rows(run, cell, order, span):
    ...
    ledger = _Ledger(cell.get("record_limit", 0))
    latencies = array("q")
...
def analyze_cell(run, cell, kind_override=None):
    ...
    elif restarted or failure is not None:
...
def compare(base, candidate, short=()):
...
def group_verdict(arms):
    ...
    comparisons = {arm: compare(arms[BASELINE], arms[arm], short) for arm in CANDIDATES}
    ...
    return {
        ...
        "incremental_cost": {
            arm: incremental(arms[INCREMENTAL[arm]], arms[arm]) for arm in CANDIDATES
        },
...
def build_groups(cells, index):
    ...
    for record in cells:
        ...
        if record["arm"] != protocol_cell["arm"]:
            record["reasons"].append("arm-differs-from-protocol")
...
def calibration(cells, runs):
    by_id = {}
    for record in cells:
        by_id.setdefault(record["cell_id"], record)
    if not by_id:
        return None
    ...
    if control is not None:
        ...
        section["interrupt_control"] = {
            ...
            "reconciles_incomplete": marker.get("status") == "incomplete",
    ...
    return [
        {
            ...
            else delay["detected"] and pace_25["admitted"] is True,
        },
        {
            ...
            if any(p is None for p in paced)
                ...
                p["admitted"] is True and p["sustained_backlog_growth"] is False
                for p in paced
            ),
        },
        {
            ...
            else gauges[GAUGE_PAIR[0]]["staged_gauges_present"]
            and not gauges[GAUGE_PAIR[1]]["staged_gauges_present"],
        },
        {
            "item": "k10 task names are 32 hex characters",
            "holds": None if not names else set(names) == {"hex32"},
        },
        {
        ...
        {
            "item": "interrupt control reconciles incomplete and is refused",
            "holds": None
            if control is None
            else control["reconciles_incomplete"] and control["steady_state_refused"],
        },
    ]
...
def render_markdown(report):
    ...
    for group in report["groups"]:
        ...
        if group["capacity"] is not None:
            ...
            lines.append(
                f"The capacity search is {group['capacity']['status']} with accepted "
                f"{state['accepted']} and rejected {state['rejected']}; the next probe "
                f"is {group['capacity']['next']}."
            )
    ...
    if report["calibration"] is not None:
        lines += ["", "## Calibration", ""]
        for item in report["calibration"]["acceptance"]:
            verdict = {True: "holds", False: "fails", None: "is not evaluable"}
            lines.append(f"The item '{item['item']}' {verdict[item['holds']]}.")
        delay = report["calibration"]["delay_control"]
        if delay is not None:
            lines.append(
                f"The delay control {'is' if delay['detected'] else 'is not'} detected "
                f"with throughput {show(delay['throughput'])} and p95 "
