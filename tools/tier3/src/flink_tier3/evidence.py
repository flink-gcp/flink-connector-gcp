...
def iter_rows(source, run_id, cell_id, hasher=None, parts=None, order=None):
    ...
    for obj in sorted(parts, key=key):
        ...
        try:
            with (
                gzip.GzipFile(fileobj=tap, mode="rb") as unzipped,
                io.TextIOWrapper(unzipped, encoding="utf-8", newline="\n") as text,
            ):
...
class Reconciler:
    ...
    def _parts(self, parts, order):
        ...
        for incarnation in order:
            ...
            if present != set(range(1, closed + 1)):
...
class Collector:
    ...
    def export(self, cell, outcome="completed"):
        ...
        if control.evidence_bytes + total > CLOUDTASKS_CEILINGS["evidence_bytes"]:
            raise Failure("Session evidence ceiling reached")
        exported = []
        for obj in reconciliation.objects:
            ...
            for key in ("crc32c", "md5"):
                if metadata.get(key) and metadata[key] != obj[key]:
                    raise Failure("Exported object differs from its source: " + name)
            exported.append(
                {
                    "name": name,
...
def release_exported(env, storage, failed=(), meter=None):
    ...
    for cell_id in env.approval.cell_ids:
        ...
        if cell_id in failed or marker is None or marker.get("cell_id") != cell_id:
            retained[cell_id] = {
                "objects": len(objects),
                "bytes": sum(int(obj["size"]) for obj in objects),
            }
            continue
        for obj in sorted(objects, key=lambda obj: _release_order(obj, prefix)):
            storage.delete(obj["name"], obj["generation"], BENCHMARK)
        env.emit(
            "benchmark-prefix-released", {"cell": cell_id, "objects": len(objects)}
        )
    return retained
