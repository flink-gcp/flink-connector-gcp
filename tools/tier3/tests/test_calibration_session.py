...
def test_hook_chain_runs_every_hook_in_order_and_stops_on_failure():
    ...
    chain = HookChain(Recording("observer"), Recording("collector"))
    ...
    calls.clear()
    # The first hook's failure must stop the chain, not merely be re-raised.
    stopping = HookChain(Recording("observer", fail=True), Recording("collector"))
    try:
        stopping.after_cell(None, {"id": "c"}, "completed")
    except rt.Failure as error:
        assert "observer failed" in str(error)
    else:
        raise AssertionError("the failing hook must stop the chain")
    assert calls == [("observer", "after_cell", "completed")]
