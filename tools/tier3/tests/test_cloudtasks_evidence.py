...
class Synthetic:
    ...
    def part(self, incarnation, index, lines, data=None):
        ...
        text = ("\n".join(lines) + "\n") if lines else ""
        # mtime 0 keeps a fixture's bytes, and so its digest, reproducible.
        self.files[name] = (
            gzip.compress(text.encode(), mtime=0) if data is None else data
        )
        return name
...
def test_verify_idle_refuses_an_exported_but_unreleased_prefix(env):
    ...
    prefix = ev.cell_prefix(RUN, CELL_A["id"])
    name = prefix + f"receipts/creator-{INC}-start.json"
    store.write_bytes(name, b"{}", rt.BENCHMARK)
    generation = store.objects(prefix, rt.BENCHMARK)[0]["generation"]
    verify_idle(environment)  # Without a marker the one-day expiry is the backstop.
    exported = {"name": name, "source_generation": generation}
    store.write(
        prefix + ev.MARKER,
        {"version": 1, "cell_id": CELL_A["id"], "objects": [exported]},
    )
    verify_idle(environment)  # A marker without this run's identity proves nothing.
    marker, marker_generation = store.read(prefix + ev.MARKER)
    store.write(prefix + ev.MARKER, {**marker, "run_id": RUN}, marker_generation)
...
def test_verify_idle_ignores_an_object_the_export_never_owned(env):
    """The release and the idle check read the same marker authorization.

    A refused or replaced object is retained by design, so treating it as an
    unreleased prefix would strand the environment lock until the benchmark
    bucket expires it a day later.
    """
    collector, environment = collector_for(env)
    store = env[1]
    synthetic = complete_cell()
    synthetic.files["receipts/creator-not-a-receipt.json"] = b"{}"
    synthetic.install(store)
    collector.export(CELL_A, "completed")
    ev.release_exported(environment, store)
    assert benchmark_names(store, synthetic.prefix) == [
        synthetic.prefix + "receipts/creator-not-a-receipt.json"
    ]
    verify_idle(environment)
...
# --- review round one: the export's ordering and its accounting -------------------


def test_export_records_its_bytes_before_the_marker_authorizes_release(env):
    collector, environment = collector_for(env)
    store = env[1]
    synthetic = complete_cell().install(store)
    marker_name = synthetic.prefix + ev.MARKER
    order = []
    write = store.write

    def recording(name, data, generation="0", bucket=rt.EVIDENCE):
        if name == marker_name:
            order.append("marker")
        if name == environment.records.path and data.get("exports"):
            order.append("record")
        return write(name, data, generation, bucket)

    store.write = recording
    collector.export(CELL_A, "completed")
    assert order[:2] == ["record", "marker"]
    control = environment.refresh()
    assert control.exports[CELL_A["id"]]["evidence_bytes"] > 0
    assert control.evidence_bytes == control.exports[CELL_A["id"]]["evidence_bytes"]


def test_a_retry_after_a_lost_marker_write_does_not_double_count(env):
    collector, environment = collector_for(env)
    store = env[1]
    synthetic = complete_cell().install(store)
    marker_name = synthetic.prefix + ev.MARKER
    ...
    def lose_the_marker(name, data, generation="0", bucket=rt.EVIDENCE):
        if name == marker_name:
            raise rt.Failure("Lost marker response")
        return write(name, data, generation, bucket)

    store.write = lose_the_marker
    with pytest.raises(rt.Failure, match="Lost marker"):
        collector.export(CELL_A, "completed")
    recorded = environment.refresh()
    assert recorded.evidence_bytes > 0
    store.write = write
    marker = collector.export(CELL_A, "completed")
    after = environment.refresh()
    assert after.evidence_bytes == recorded.evidence_bytes  # not counted twice
    assert after.exports[CELL_A["id"]]["manifest_sha256"] == marker["manifest_sha256"]


def test_an_export_without_a_destination_checksum_is_refused(env):
    collector, environment = collector_for(env)
    store = env[1]
    complete_cell().install(store)
    metadata = store.metadata

    def without_checksums(name, bucket=rt.EVIDENCE):
        found = metadata(name, bucket)
        if found is not None and bucket == rt.EVIDENCE:
            found = {k: v for k, v in found.items() if k not in ("crc32c", "md5")}
        return found

    store.metadata = without_checksums
    with pytest.raises(rt.Failure, match="reports no checksum"):
        collector.export(CELL_A, "completed")
    assert environment.refresh().exports == {}


def test_an_export_whose_destination_keeps_its_size_is_still_refused(env):
    collector, environment = collector_for(env)
    store = env[1]
    synthetic = complete_cell().install(store)
    target = synthetic.prefix + f"receipts/creator-{INC}-start.json"
    rewrite = store.rewrite

    def corrupt(source_bucket, name, generation, destination, bucket=rt.EVIDENCE):
        result = rewrite(source_bucket, name, generation, destination, bucket)
        if name == target:
            original = store.blobs[bucket, destination][0]
            store.blobs[bucket, destination] = (
                bytes([original[0] ^ 0xFF]) + original[1:],
                store.blobs[bucket, destination][1],
            )
        return result

    store.rewrite = corrupt
    with pytest.raises(rt.Failure, match="differs from its source"):
        collector.export(CELL_A, "completed")
    assert environment.refresh().exports == {}


def test_an_export_whose_destination_keeps_its_checksum_is_still_refused(env):
    collector, environment = collector_for(env)
    store = env[1]
    synthetic = complete_cell().install(store)
    target = synthetic.prefix + f"receipts/creator-{INC}-start.json"
    ...
    def shortened(name, bucket=rt.EVIDENCE):
        found = metadata(name, bucket)
        if found is not None and bucket == rt.EVIDENCE and name == target:
            found = {**found, "size": str(int(found["size"]) - 1)}
        return found

    store.metadata = shortened
    with pytest.raises(rt.Failure, match="differs from its source"):
        collector.export(CELL_A, "completed")
    assert environment.refresh().exports == {}


def test_a_prefix_is_released_only_behind_a_complete_marker(env):
    collector, environment = collector_for(env)
    store = env[1]
    synthetic = complete_cell().install(store)
    collector.export(CELL_A, "completed")
    marker_name = synthetic.prefix + ev.MARKER
    complete = store.read(marker_name)[0]
    for change in ({"version": 2}, {"run_id": "other"}, {"cell_id": "other"}):
        _marker, generation = store.read(marker_name)
        store.write(marker_name, {**complete, **change}, generation)
        retained = ev.release_exported(environment, store)
        assert retained[CELL_A["id"]]["objects"] == len(synthetic.files) + 1
        assert benchmark_names(store, synthetic.prefix)
    _marker, generation = store.read(marker_name)
    store.write(marker_name, complete, generation)
    assert ev.release_exported(environment, store) == {}
    assert benchmark_names(store, synthetic.prefix) == []


def test_the_marker_is_the_last_object_a_release_deletes(env):
    collector, environment = collector_for(env)
    store = env[1]
    synthetic = complete_cell().install(store)
    store.write(
        synthetic.prefix + "state/chk-1/_metadata", {"chk": 1}, bucket=rt.BENCHMARK
    )
    collector.export(CELL_A, "completed")
    deleted = []
    delete = store.delete

    def recording(name, generation, bucket=rt.EVIDENCE):
        deleted.append(name[len(synthetic.prefix) :])
        return delete(name, generation, bucket)

    store.delete = recording
    ev.release_exported(environment, store)
    assert deleted[0].startswith("state/")
    assert deleted[-1] == ev.MARKER  # nothing is orphaned by a partial release
    assert {n.split("/")[0] for n in deleted[1:-1]} == {"rows", "receipts"}


# --- review round three: authorized release and bounded parsing ----------------


def test_a_release_deletes_only_what_the_marker_authorized(env):
    collector, environment = collector_for(env)
    store = env[1]
    synthetic = complete_cell()
    # The reconciliation refuses this object and records the reason; a release
    # that removed it would leave the exported evidence unable to show why.
    synthetic.files["receipts/creator-not-a-receipt.json"] = b"{}"
    synthetic.install(store)
    marker = collector.export(CELL_A, "completed")
    assert "unexpected-object" in marker["reconciliation"]["reasons"]
    retained = ev.release_exported(environment, store)
    assert retained[CELL_A["id"]]["unauthorized"] == [
        "receipts/creator-not-a-receipt.json"
    ]
    assert benchmark_names(store, synthetic.prefix) == [
        synthetic.prefix + "receipts/creator-not-a-receipt.json"
    ]
    # The receipt counts what was deleted and what stayed, not the listing:
    # every file but the refused one, plus the benchmark copy of the marker.
    exported = len(synthetic.files) - 1
    [released] = events(env, "benchmark-prefix-released")
    assert released == {
        "cell": CELL_A["id"],
        "objects": exported + 1,
        "retained": 1,
...
def test_an_object_replaced_after_its_export_survives_the_release(env):
    collector, environment = collector_for(env)
    store = env[1]
    synthetic = complete_cell().install(store)
    collector.export(CELL_A, "completed")
    replaced = synthetic.prefix + f"rows/{INC}-000001.csv.gz"
    body, generation = store.blobs[rt.BENCHMARK, replaced]
    store.blobs[rt.BENCHMARK, replaced] = (body, str(int(generation) + 1))
    retained = ev.release_exported(environment, store)
    # The exported copy is of the generation that was read and verified, so the
    # replacement is a different object and the release has no authority over it.
    assert retained[CELL_A["id"]]["unauthorized"] == [f"rows/{INC}-000001.csv.gz"]
    assert benchmark_names(store, synthetic.prefix) == [replaced]


def test_a_receipt_count_no_session_could_produce_is_refused():
    synthetic = complete_cell()
    synthetic.creator(INC, rows=6, parts=1_000_000_000)
    result = reconcile(synthetic)
    assert result.status == "invalid"
    assert "receipt-counts-out-of-range" in result.reasons
    synthetic = complete_cell()
    synthetic.creator(INC, rows=-1, parts=3)
    assert "receipt-counts-out-of-range" in reconcile(synthetic).reasons


def test_a_part_without_a_newline_is_refused_before_it_is_buffered():
    synthetic = complete_cell()
    long_line = b"x" * (ev.ROW_CEILING + ev.READ_CHUNK + 1)
    synthetic.files[f"rows/{INC}-000001.csv.gz"] = gzip.compress(long_line)
    with pytest.raises(rt.Failure, match="over-long line"):
        reconcile(synthetic)


def test_a_long_line_is_refused_at_its_own_length_not_at_a_chunk_boundary():
    """The bound is the row ceiling, which is what the constant claims."""
    synthetic = complete_cell()
    over = b"x" * (ev.ROW_CEILING + 1) + b"\n" + b"short\n"
    synthetic.files[f"rows/{INC}-000001.csv.gz"] = gzip.compress(over)
    with pytest.raises(rt.Failure, match="over-long line"):
        reconcile(synthetic)
    stream = io.BytesIO(b"x" * ev.ROW_CEILING + b"\n")
    assert [len(line) for line in ev._lines(stream, "part")] == [ev.ROW_CEILING]
